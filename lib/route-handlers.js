...
var clientId = (process.env.CLIENT_ID || config.oidc.clientId);
var clientSecret = (process.env.CLIENT_SECRET || config.oidc.clientSecret);
var redirectUri = (process.env.REDIRECT_URI || config.oidc.redirectUri);
var oktaUrl = (process.env.AUTHZ_SERVER || config.oidc.oktaUrl);
...
handlers.callback = (req, res) => {
  ...
  const options = {
    url: `${oktaUrl}/oauth2/v1/token?${query}`,
    method: 'POST',
    headers: {
  ...
  request(options, (err, tokenRes, json) => {
    ...
    .then((jwk) => {
      ...
      if (oktaUrl !== claims.iss) {
        res.status(401).send(`id_token issuer ${claims.iss} does not match our issuer ${oktaUrl}`);
        return;
      }
